...
public sealed class ArtifactTests : IDisposable
{
    ...
    public void ManifestMustBeBoundedUniqueAsciiAndSafe()
    {
        ...
        Assert.Equal(["one", "two"], ArtifactSnapshots.LoadManifest(path));
        File.WriteAllText(path, "one\none\n", Encoding.ASCII);
        Assert.Throws<ArtifactException>(() => ArtifactSnapshots.LoadManifest(path));
        File.WriteAllText(path, "../unsafe\n", Encoding.ASCII);
        Assert.Throws<ArtifactException>(() => ArtifactSnapshots.LoadManifest(path));
        File.WriteAllBytes(path, [0xc3, 0xa9, (byte)'\n']);
        Assert.Throws<ArtifactException>(() => ArtifactSnapshots.LoadManifest(path));
        File.WriteAllBytes(path, new byte[4097]);
        Assert.Throws<ArtifactException>(() => ArtifactSnapshots.LoadManifest(path));
        File.WriteAllText(path, string.Join('\n', Enumerable.Range(0, 33).Select(index => $"f{index}")), Encoding.ASCII);
        Assert.Throws<ArtifactException>(() => ArtifactSnapshots.LoadManifest(path));
    }
