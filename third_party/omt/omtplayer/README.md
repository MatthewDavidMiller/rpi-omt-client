...
**Important:** The service needs to run as a user service in order to access pulseaudio with low latency performance.

1. Copy the omtplayer app and libraries into a dedicated folder for the service to run from.

```
sudo mkdir /opt/omtplayer
sudo cp ~/omtplayer/build/arm64/* /opt/omtplayer/
sudo chown $(whoami):$(whoami) /opt/omtplayer/*
```

2. Copy the omtplayer.service template into the user service folder.

```
mkdir -p ~/.config/systemd/user/
sudo cp ~/omtplayer/omtplayer.service ~/.config/systemd/user/
```

3. Make sure user service is available on boot, reload systemctl and enable the service

```
sudo loginctl enable-linger $(whoami)
systemctl --user daemon-reload
systemctl --user enable omtplayer
```

4. Start the service and check its status

```
systemctl --user start omtplayer
systemctl --user status omtplayer
```
