...
internal class ALSAPlayer : IDisposable
{
    ...
    public const uint DEFAULT_LATENCY_MS = 80;
    ...
    [DllImport(Lib)]
    private static extern int snd_pcm_delay(IntPtr pcm, ref int fdelay);

    [DllImport(Lib)]
    private static extern int snd_pcm_get_params(IntPtr pcm, ref uint buffer_size, ref uint period_size);

    [DllImport(Lib)]
    private static extern int snd_pcm_recover(IntPtr pcm, int err, int silent);

    public ALSAPlayer(string deviceName, uint sampleRate, uint channels, uint latencyMs)
    {
        ...
        hr = snd_pcm_set_params(pcmHandle, _snd_pcm_format_t.SND_PCM_FORMAT_FLOAT_LE, _snd_pcm_access_t.SND_PCM_ACCESS_RW_INTERLEAVED, channels, sampleRate, 1, latencyMs * 1000);
        if (hr != 0)
        {
    ...
    public int GetBufferAvailable()
    {
        ...
        if (available < 0)
        {
            ...
            if (available == -EPIPE || available == -ESTRPIPE)
            {
                ...
                Console.WriteLine("ALSA.RecoveredStream: " + err + "|" + available);
            }
        }
        return available;
    }
    ...
    {
        int fDelay = 0;
        int err = snd_pcm_delay(pcmHandle, ref fDelay);
        if (err == 0)
        {
            return (fDelay * 1000) / (int)sampleRate;
        }
        return 0;
    }
