...
- Prefer minimal, targeted changes. This repo already has strong test coverage around the Rust Web app, entrypoint, install flow, and container behavior.
...
- Rust Web app or templates: `make test-web` and, when behavior changes materially, `make test`
...
- `crates/omt-web/src/app.rs`: HTTPS composition, routes, sessions, CSRF, and rate limits
- `crates/omt-web/src/playback.rs`: source discovery, playback, and status projection
- `crates/omt-web/src/diagnostics.rs`: diagnostics, support archives, and host actions
- `crates/omt-web/src/auth.rs`: credentials and persistent sessions
- `crates/omt-web/src/state.rs`: bounded state and atomic OMT target persistence
- `crates/omt-web/templates/` and `crates/omt-web/static/`: web UI
...
- `tests/schema/`: shared native contract vectors
