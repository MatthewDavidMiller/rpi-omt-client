...
except ImportError:  # The container exposes /app as the Python import root.
    from settings import AppSettings, load_settings
...
def create_app(
    ...
    for endpoint in (
        ...
        app.view_functions[endpoint] = limiter.limit(effective_settings.debug_action_limit)(
            app.view_functions[endpoint]
        )
    app.view_functions["diagnostics.download_bundle"] = limiter.limit(
        effective_settings.debug_download_limit
    )(app.view_functions["diagnostics.download_bundle"])
    app.view_functions["system.reboot"] = limiter.limit(
        effective_settings.reboot_action_limit
