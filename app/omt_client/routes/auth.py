...
def login():
    if request.method == "POST":
        try:
            session_id = services().auth.authenticate(
                request.form.get("password", ""), session.get("session_id")
            )
        except Exception:
        ...
        if session_id:
            ...
            password_digest = getattr(services().auth, "password_digest", "")
            if password_digest:
                session["password_digest"] = password_digest
