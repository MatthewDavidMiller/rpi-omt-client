...
assert_contains '^[[:space:]]*mem_limit:[[:space:]]+"\$\{OMT_CONTAINER_MEMORY_LIMIT:-512m\}"[[:space:]]*$' \
    "container memory is bounded for low-RAM Pi models"
assert_contains '^[[:space:]]*memswap_limit:[[:space:]]+"\$\{OMT_CONTAINER_MEMORY_LIMIT:-512m\}"[[:space:]]*$' \
