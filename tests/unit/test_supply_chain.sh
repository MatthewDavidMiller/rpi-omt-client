...
for project in App Core Tests IntegrationTests; do
    lock_file="${PROJECT_ROOT}/deployer/RpiOmt.Deployer.${project}/packages.lock.json"
...
assert_literal "${PYTHON_AUDIT}" 'run_audit audit_hash_locked "app/requirements.txt"' "Python audit covers application dependencies"
...
assert_literal "${PROJECT_ROOT}/Dockerfile" 'dotnet publish receiver/RpiOmt.Receiver/RpiOmt.Receiver.csproj' "Container builds the pinned OMT receiver"
assert_literal "${PROJECT_ROOT}/receiver/RpiOmt.Receiver/packages.lock.json" '"libomtnet"' "Receiver NuGet graph is locked"
assert_literal "${PROJECT_ROOT}/Dockerfile" '--require-hashes' "Container Python install is hash locked"
assert_literal "${PROJECT_ROOT}/Dockerfile" 'runtime-sbom.cdx.json' "Container emits a runtime SBOM"
...
assert_literal "${PROJECT_ROOT}/deployer/RpiOmt.Deployer.Core/DeploymentOperations.cs" 'tonistiigi/binfmt@sha256:400a4873b838d1b89194d982c45e5fb3cda4593fbfd7e08a02e76b03b21166f0' "Deployer pins the privileged binfmt image"
