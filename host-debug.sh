#!/bin/bash
# Collect a request-correlated, wall-budgeted host snapshot for the Debug bundle.
...
OUTPUT_FILE="${OMT_HOST_DEBUG_OUTPUT:-/var/lib/omt-client/host-debug.txt}"
REQUEST_FILE="${OMT_HOST_DEBUG_REQUEST_FILE:-/var/lib/omt-client/request}"
INSTALL_DIR="${OMT_INSTALL_DIR:-/opt/omt-client}"
HOST_DEBUG_BUDGET_SECONDS="${OMT_HOST_DEBUG_BUDGET_SECONDS:-25}"
OUTPUT_DIR="$(dirname "${OUTPUT_FILE}")"
PCAP_OUTPUT_FILE="${OMT_HOST_DEBUG_PCAP_OUTPUT:-${OUTPUT_DIR}/host-network.pcap}"
PCAP_METADATA_FILE="${OMT_HOST_DEBUG_PCAP_METADATA_OUTPUT:-${OUTPUT_DIR}/host-network-pcap.txt}"
PCAP_MAX_BYTES=67108864
HOST_SECTION_MAX_BYTES=262144
HOST_REQUEST_MAX_BYTES=4096

if [[ ! "${HOST_DEBUG_BUDGET_SECONDS}" =~ ^[1-9][0-9]*$ ]]; then
    echo "Invalid OMT_HOST_DEBUG_BUDGET_SECONDS: ${HOST_DEBUG_BUDGET_SECONDS}" >&2
    exit 1
fi
           ...
           (( request_size <= HOST_REQUEST_MAX_BYTES )); then
            ...
            if [[ "${request_before}" == "${request_after}" && \
                ...
                for request_line in "${request_lines[@]}"; do
                    if [[ "${request_line}" == request_id=* ]]; then
                        request_id_candidate="${request_line#request_id=}"
                        ((request_id_count += 1))
                    fi
                done
                if (( request_id_count == 1 )) && \
                   [[ "${request_id_candidate}" =~ ^[A-Za-z0-9._-]{1,128}$ ]]; then
...
remaining_seconds() {
    local remaining=$((HOST_DEBUG_BUDGET_SECONDS - SECONDS))
...
start_packet_captures() {
    local capture_seconds=$((HOST_DEBUG_BUDGET_SECONDS - 2))
...
publish_pcap() {
    ...
    if [[ "${PCAP_EXIT_STATUS}" == "not_started" ]]; then
...

{
    section "metadata"
    printf 'request_id=%s\n' "${REQUEST_ID}"
    printf 'host_debug_budget_seconds=%s\n' "${HOST_DEBUG_BUDGET_SECONDS}"
    ...
    run "service definitions (sanitized)" sh -c \
        'systemctl cat omt-client.service omt-client-avahi-proxy.service omt-client-host-debug.service omt-client-host-debug.path 2>&1 | sed -E "s/(([A-Za-z0-9_]*(PASSWORD|SECRET|TOKEN|CREDENTIAL|PRIVATE_KEY)[A-Za-z0-9_]*)=)[^[:space:]]+/\1<redacted>/Ig"'
    ...
    run "host diagnostics trigger status" systemctl status omt-client-host-debug.path --no-pager -l
    ...
    run "container resource state" sh -c \
        'cd "$1" && cid=$(docker compose ps -q omt-client 2>/dev/null || true); if [ -n "$cid" ]; then docker stats --no-stream --format "name={{.Name}} cpu={{.CPUPerc}} memory={{.MemUsage}} pids={{.PIDs}} network={{.NetIO}} block={{.BlockIO}}" "$cid"; else echo no_container; fi' \
        sh "${INSTALL_DIR}"
    ...
    run "container supplemental groups versus devices" sh -c \
        'cd "$1" && cid=$(docker compose ps -q omt-client 2>/dev/null || true); if [ -n "$cid" ]; then docker exec "$cid" sh -c '\''id; for root in /dev/dri /dev/snd; do find "$root" -maxdepth 1 -type c -exec stat -c "path=%n uid=%u gid=%g mode=%a" {} + 2>/dev/null; done'\''; else echo no_container; fi' \
        sh "${INSTALL_DIR}"
    run "deployed artifact hashes" sh -c \
        'cd "$1" || exit; for file in docker-compose.yml install.sh uninstall.sh host-debug.sh omt-client-arm64.tar.gz; do if [ -f "$file" ]; then sha256sum "$file"; else printf "missing=%s\n" "$file"; fi; done' \
        sh "${INSTALL_DIR}"
    ...
    run "container filtered D-Bus check" sh -c \
        'cd "$1" && cid=$(docker compose ps -q omt-client 2>/dev/null || true); if [ -z "$cid" ]; then echo no_container; exit; fi; docker exec "$cid" sh -c '\''printf "DBUS_SYSTEM_BUS_ADDRESS=%s\n" "${DBUS_SYSTEM_BUS_ADDRESS:-<unset>}"; stat -Lc "socket_type=%F uid=%u gid=%g mode=%a" /host-debug/avahi-system-bus 2>&1; timeout 5 dbus-send --bus="${DBUS_SYSTEM_BUS_ADDRESS}" --type=method_call --print-reply --dest=org.freedesktop.Avahi / org.freedesktop.Avahi.Server.GetVersionString 2>&1'\''' \
        sh "${INSTALL_DIR}"
    ...
    printf 'budget_exhausted=%s\n' "$(( SECONDS >= HOST_DEBUG_BUDGET_SECONDS ))"
} > "${TMP_FILE}"
