...
{
    [Fact]
    public async Task VersionDetectionReadsProcessEnvironmentWhenNoMapIsSupplied()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                "v-process",
                await new VersionDetector(new ScriptedRunner()).DetectAsync(
                    "/src/project",
                    null,
...
public sealed class DeploymentOperationTests : IDisposable
{
    ...
    public DeploymentOperationTests()
    {
        ...
        foreach (var name in Manifest.Split('\n', StringSplitOptions.RemoveEmptyEntries).Skip(1))
        {
            var path = Path.Combine(_project, name);
            Directory.CreateDirectory(Path.GetDirectoryName(path)!);
            File.WriteAllText(path, name);
        }
        File.WriteAllText(Path.Combine(_project, "deploy", "manifest-v2.txt"), Manifest);
    }
    ...
    public async Task DeploymentValidationRejectsUnsafeOrIncompleteProjects()
    {
        ...
        var manifest = Path.Combine(_project, "deploy", "manifest-v2.txt");
        File.WriteAllText(manifest, "version=2\none\ntwo\n");
        ...
        await Assert.ThrowsAsync<DeploymentException>(() =>
            operations.DeployAsync(Snapshot(), CancellationToken.None));
        File.WriteAllText(
            manifest,
        ...
        File.Delete(Path.Combine(_project, "deploy", "host", "host-diagnostics.sh"));
        await Assert.ThrowsAsync<DeploymentException>(() => operations.DeployAsync(Snapshot(), CancellationToken.None));
    }
    ...
    [InlineData(
        "version=2\nomt-client-arm64.tar.gz\ndeploy/transaction.sh\n")]
    public async Task ManifestDriftBeforeUploadFailsClosed(string replacement)
    {
        string manifest = Path.Combine(_project, "deploy", "manifest-v2.txt");
        var remote = new FakeRemote
        {
            MutateManifestPath = manifest,
            ManifestReplacement = replacement,
        };
        await Assert.ThrowsAsync<DeploymentException>(() =>
    ...
    public async Task MalformedDigestUploadFailureAndMutationFailClosed()
    {
        ...
        Assert.Contains(malformed.Commands, call => call.Command.StartsWith("if [ -d", StringComparison.Ordinal));
...
internal sealed class FakeRemote : IRemoteClient
{
    ...
    public string? MutateManifestPath { get; init; }
    public string? ManifestReplacement { get; init; }
    ...
    public Task<CommandResult> RunAsync(string command, string input, Action<string>? onOutput, TimeSpan timeout, CancellationToken cancellationToken)
    {
        ...
        if (ThrowDuringCleanup && command.StartsWith("if [ -d", StringComparison.Ordinal))
        {
        ...
        if (command.Contains(" install -d ", StringComparison.Ordinal) &&
            MutateManifestPath is not null &&
            ManifestReplacement is not null)
        {
            File.WriteAllText(MutateManifestPath, ManifestReplacement);
        }
        ...
        if (FailCleanup && command.StartsWith("if [ -d", StringComparison.Ordinal))
        {
