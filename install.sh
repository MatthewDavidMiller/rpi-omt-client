...
HOST_DEBUG_SCRIPT="${INSTALL_DIR}/host-debug.sh"
HOST_REBOOT_SCRIPT="${INSTALL_DIR}/host-reboot.sh"
...
DEPLOY_ARTIFACT_MANIFEST="${INSTALL_DIR}/deploy-artifacts.txt"
...
HOST_ACTION_STATE_DIR="${HOST_DEBUG_STATE_DIR}/host-actions"
...
for required_file in "${TARBALL}" "${COMPOSE_FILE}" "${HOST_DEBUG_SCRIPT}" \
...
install -d -m 0755 "${HOST_DEBUG_INSTALLED_DIR}"
chown root:root "${HOST_DEBUG_INSTALLED_DIR}"
publish_file "${HOST_DEBUG_INSTALLED_SCRIPT}" 0755 root root < "${HOST_DEBUG_SCRIPT}"
publish_file "${HOST_REBOOT_INSTALLED_SCRIPT}" 0755 root root < "${HOST_REBOOT_SCRIPT}"
publish_file "${DEPLOY_RECOVERY_HELPER}" 0755 root root < "${DEPLOY_TRANSACTION_SCRIPT}"
publish_file "${DEPLOY_RECOVERY_MANIFEST}" 0644 root root < "${DEPLOY_ARTIFACT_MANIFEST}"
systemctl stop omt-client-avahi-proxy.service 2>/dev/null || true
rm -f "${AVAHI_PROXY_SOCKET}"
install -d -m 2750 "${HOST_DEBUG_STATE_DIR}"
chown "root:${OMT_GID}" "${HOST_DEBUG_STATE_DIR}"
touch "${HOST_DEBUG_STATE_DIR}/request" "${HOST_DEBUG_STATE_DIR}/host-debug.txt"
chown "${OMT_UID}:${OMT_GID}" "${HOST_DEBUG_STATE_DIR}/request"
chown "root:${OMT_GID}" "${HOST_DEBUG_STATE_DIR}/host-debug.txt"
chmod 600 "${HOST_DEBUG_STATE_DIR}/request"
chmod 640 "${HOST_DEBUG_STATE_DIR}/host-debug.txt"
...
systemctl stop omt-client-host-debug.timer 2>/dev/null || true
systemctl disable omt-client-host-debug.timer 2>/dev/null || true
rm -f "${LEGACY_HOST_DEBUG_TIMER_FILE}"

publish_file "${AVAHI_PROXY_SERVICE_FILE}" 0644 root root <<EOF
[Unit]
...
ReadWritePaths=${HOST_DEBUG_STATE_DIR}

[Install]
WantedBy=multi-user.target
EOF

publish_file /etc/systemd/system/omt-client-host-debug.service 0644 root root <<EOF
[Unit]
...
Environment=OMT_HOST_DEBUG_BUDGET_SECONDS=25
ExecStart=${HOST_DEBUG_INSTALLED_SCRIPT}
UMask=0027
NoNewPrivileges=true
PrivateTmp=true
...
publish_file /etc/systemd/system/omt-client-host-debug.path 0644 root root <<'EOF'
[Unit]
Description=Run OMT Client host diagnostics on web request

[Path]
PathChanged=/var/lib/omt-client/request
Unit=omt-client-host-debug.service

[Install]
WantedBy=multi-user.target
EOF

publish_file "${HOST_REBOOT_SERVICE_FILE}" 0644 root root <<EOF
[Unit]
...
publish_file "${HOST_REBOOT_PATH_FILE}" 0644 root root <<EOF
[Unit]
...
publish_file /etc/systemd/system/omt-client.service 0644 root root <<EOF
[Unit]
...
systemctl enable --now omt-client-host-debug.path
